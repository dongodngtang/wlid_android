 ...
 */
public class UserRecordAdapter extends MultiItemCommonAdapter<BirdRecordTotalItem> {
    ...
    public UserRecordAdapter(Context context, List<BirdRecordTotalItem> datas) {
    ...
    public void convert(ViewHolder holder, final BirdRecordTotalItem birdRecordTotalItem) {
        ...
        //绑定二级列表适配器
        expandableListView.setAdapter(new UserRecordCityAdapter(birdRecordTotalItem.getArea_list(),isLookMeRecord));
...
//        }
        ...
        String strTime="";
        if (!TextUtils.isEmpty(birdRecordTotalItem.getTime())){
         strTime=birdRecordTotalItem.getTime().replace('.','-');
        }
        String shareUrl = String.format(SHARE_URL_FORMAT, mShareUrl, strTime);
